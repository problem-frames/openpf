...
import uk.ac.open.rbac.rbac.diagram.part.RBACDiagramUpdater;
import uk.ac.open.rbac.rbac.diagram.part.RBACLinkDescriptor;
import uk.ac.open.rbac.rbac.diagram.part.RBACNodeDescriptor;
...
public class ModelCanonicalEditPolicy extends CanonicalEditPolicy {
	...
	protected Set getFeaturesToSynchronize() {
		if (myFeaturesToSynchronize == null) {
			...
			myFeaturesToSynchronize.add(RbacPackage.eINSTANCE.getModel_Roles());
			myFeaturesToSynchronize.add(RbacPackage.eINSTANCE.getModel_Users());
			myFeaturesToSynchronize.add(RbacPackage.eINSTANCE
					.getModel_Sessions());
			myFeaturesToSynchronize.add(RbacPackage.eINSTANCE
					.getModel_Objects());
		}
	...
	protected List getSemanticChildrenList() {
		...
		List<RBACNodeDescriptor> childDescriptors = RBACDiagramUpdater
				.getModel_1000SemanticChildren(viewObject);
		for (RBACNodeDescriptor d : childDescriptors) {
	...
	private boolean isMyDiagramElement(View view) {
		int visualID = RBACVisualIDRegistry.getVisualID(view);
		switch (visualID) {
	...
	protected void refreshSemantic() {
		...
		for (Iterator<RBACNodeDescriptor> descriptorsIterator = childDescriptors
				.iterator(); descriptorsIterator.hasNext();) {
			RBACNodeDescriptor next = descriptorsIterator.next();
			String hint = RBACVisualIDRegistry.getType(next.getVisualID());
		...
		for (RBACNodeDescriptor next : childDescriptors) {
			String hint = RBACVisualIDRegistry.getType(next.getVisualID());
	...
	private Collection<IAdaptable> refreshConnections() {
		...
		Collection<RBACLinkDescriptor> linkDescriptors = collectAllLinks(
				getDiagram(), domain2NotationMap);
		Collection existingLinks = new LinkedList(getDiagram().getEdges());
		for (Iterator linksIterator = existingLinks.iterator(); linksIterator
				.hasNext();) {
			Edge nextDiagramLink = (Edge) linksIterator.next();
			int diagramLinkVisualID = RBACVisualIDRegistry
			...
			for (Iterator<RBACLinkDescriptor> linkDescriptorsIterator = linkDescriptors
					.iterator(); linkDescriptorsIterator.hasNext();) {
				RBACLinkDescriptor nextLinkDescriptor = linkDescriptorsIterator
						.next();
	 ...
	 * @generated
	 */
	private Collection<RBACLinkDescriptor> collectAllLinks(View view,
		...
		case UserEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(RBACDiagramUpdater
						.getUser_2002ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
		...
		case SessionEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(RBACDiagramUpdater
						.getSession_2003ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
		...
		case ObjectEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(RBACDiagramUpdater
						.getObject_2004ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
		...
		case UserRoleAssignmentEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(RBACDiagramUpdater
						.getUserRoleAssignment_4001ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
	...
	private Collection<IAdaptable> createConnections(
			Collection<RBACLinkDescriptor> linkDescriptors,
			Map<EObject, View> domain2NotationMap) {
		LinkedList<IAdaptable> adapters = new LinkedList<IAdaptable>();
		for (RBACLinkDescriptor nextLinkDescriptor : linkDescriptors) {
			...
			CreateConnectionViewRequest.ConnectionViewDescriptor descriptor = new CreateConnectionViewRequest.ConnectionViewDescriptor(
					...
					RBACVisualIDRegistry.getType(nextLinkDescriptor
