...
import uk.ac.open.istar.services.IstarGrammarAccess.DependencyElements;

public final class IstarDependencyConsumer extends NonTerminalConsumer {

	private DependencyElements rule;	
	...
	protected class CrossReference$3$Consumer extends ElementConsumer<CrossReference> {
		...
		protected int doConsume(boolean optional) throws Exception {
			return consumeTerminal(idConsumer, "dependencyFrom", false, false, getElement(), getRuleCall$5$Delimiter(), optional);
		}
	}
	...
	protected class CrossReference$8$Consumer extends ElementConsumer<CrossReference> {
		...
		@Override
		protected int doConsume(boolean optional) throws Exception {
			return consumeTerminal(idConsumer, "dependencyTo", false, false, getElement(), getRuleCall$10$Delimiter(), optional);
		}
	}

	public IstarDependencyConsumer(INonTerminalConsumerConfiguration configuration, ITerminalConsumer[] hiddenTokens) {
	...
	public DependencyElements getRule() {
		return rule;
	}
	
	public void setRule(DependencyElements rule) {
		this.rule = rule;
		
		group$1$Consumer = new Group$1$Consumer(rule.getGroup());
		assignment$2$Consumer = new Assignment$2$Consumer(rule.getDependencyFromAssignment_0());
		crossReference$3$Consumer = new CrossReference$3$Consumer(rule.getDependencyFromDependableCrossReference_0_0());
		keyword$6$Consumer = new Keyword$6$Consumer(rule.getTildeGreaterThanSignKeyword_1());
		assignment$7$Consumer = new Assignment$7$Consumer(rule.getDependencyToAssignment_2());
		crossReference$8$Consumer = new CrossReference$8$Consumer(rule.getDependencyToDependableCrossReference_2_0());
	}
