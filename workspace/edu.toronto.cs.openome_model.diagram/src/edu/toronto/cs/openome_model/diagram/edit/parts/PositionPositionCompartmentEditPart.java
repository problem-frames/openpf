...
public class PositionPositionCompartmentEditPart extends
	...
	protected void handleNotificationEvent(Notification event) {
		...
		if (NotationPackage.eINSTANCE.getSize_Width().equals(feature)
			...
			IGraphicalEditPart positionEdit = (IGraphicalEditPart) getParent();
			((PositionEditPart) positionEdit).getPrimaryShape().setMinimumContraction(minimumContraction);
			...
			if(((Boolean) getStructuralFeatureValue(NotationPackage.eINSTANCE
					.getDrawerStyle_Collapsed())).booleanValue() || 
					children.size() == 0){
				((PositionEditPart) positionEdit).getPrimaryShape().setMinimumContraction(
