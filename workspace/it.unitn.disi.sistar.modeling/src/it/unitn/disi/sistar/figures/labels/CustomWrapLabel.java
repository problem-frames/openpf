package it.unitn.disi.sistar.figures.labels;
 ...
 * @author melaasar
 */
public class CustomWrapLabel
	extends WrapLabel  // note: old base class is WrapLabel
	 ...
	 * @since 2.0
	 */
	public CustomWrapLabel(String s) {
		if (s != null) {
			text = s;
		} else {
			text = "";//$NON-NLS-1$
...
//		setBorder(new LineBorderEx(ColorConstants.red,3));
	 ...
	 * @since 2.0
	 */
	public CustomWrapLabel(Image i) {
		text = "";//$NON-NLS-1$
	 ...
	 * @since 2.0
	 */
	public CustomWrapLabel(String s, Image i) {
		if (s != null) {
			text = s;
		} else {
			text = "";//$NON-NLS-1$
	...
	public Dimension getPreferredSize(int wHint, int hHint) {
		...
		preferredTextSize = prefSize.getCopy(); // minh sang
	...
	public void paintFigure(Graphics graphics) {
		...
		if (subString.length() > 0) {
			if (!isEnabled()) {
				...
				paintCustomText(graphics, subString);
				graphics.translate(-1, -1);
	 ...
	 */	
	private void paintCustomText(Graphics graphics, String subString) {		
	...
	public void setText(String s) {
		...
		if (s == null)
			s = "";//$NON-NLS-1$
		if (s.equals(text))  // bug: WrappingLabel's constructor calls setText(""), at that time, text is null
			return;
		text = s;
		revalidate();
