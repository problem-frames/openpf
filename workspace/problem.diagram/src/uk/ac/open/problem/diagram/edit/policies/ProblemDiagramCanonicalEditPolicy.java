...
public class ProblemDiagramCanonicalEditPolicy extends
	...
	protected EStructuralFeature getFeatureToSynchronize() {
		return ProblemPackage.eINSTANCE.getProblemDiagram_Nodes();
	}
	...
	protected List getSemanticChildrenList() {
		...
		List<ProblemNodeDescriptor> childDescriptors = ProblemDiagramUpdater
				.getProblemDiagram_1000SemanticChildren(viewObject);
	...
	protected void refreshSemantic() {
		...
		List<ProblemNodeDescriptor> childDescriptors = ProblemDiagramUpdater
				.getProblemDiagram_1000SemanticChildren((View) getHost()
						.getModel());
		...
		HashMap<ProblemNodeDescriptor, LinkedList<View>> potentialViews = new HashMap<ProblemNodeDescriptor, LinkedList<View>>();
		//
		// iteration happens over list of desired semantic elements, trying to find best matching View, while original CEP
		// iterates views, potentially losing view (size/bounds) information - i.e. if there are few views to reference same EObject, only last one 
		// to answer isOrphaned == true will be used for the domain element representation, see #cleanCanonicalSemanticChildren()
		for (Iterator<ProblemNodeDescriptor> descriptorsIterator = childDescriptors
				.iterator(); descriptorsIterator.hasNext();) {
			ProblemNodeDescriptor next = descriptorsIterator.next();
			String hint = ProblemVisualIDRegistry.getType(next.getVisualID());
		...
		for (ProblemNodeDescriptor next : childDescriptors) {
			String hint = ProblemVisualIDRegistry.getType(next.getVisualID());
	...
	private Collection<IAdaptable> refreshConnections() {
		...
		Collection<ProblemLinkDescriptor> linkDescriptors = collectAllLinks(
		...
		for (Iterator linksIterator = existingLinks.iterator(); linksIterator
			...
			for (Iterator<ProblemLinkDescriptor> linkDescriptorsIterator = linkDescriptors
					.iterator(); linkDescriptorsIterator.hasNext();) {
				ProblemLinkDescriptor nextLinkDescriptor = linkDescriptorsIterator
						.next();
	 ...
	 * @generated
	 */
	private Collection<ProblemLinkDescriptor> collectAllLinks(View view,
	...
	private Collection<IAdaptable> createConnections(
			Collection<ProblemLinkDescriptor> linkDescriptors,
			Map<EObject, View> domain2NotationMap) {
		LinkedList<IAdaptable> adapters = new LinkedList<IAdaptable>();
		for (ProblemLinkDescriptor nextLinkDescriptor : linkDescriptors) {
			...
			CreateConnectionViewRequest.ConnectionViewDescriptor descriptor = new CreateConnectionViewRequest.ConnectionViewDescriptor(
					...
					ProblemVisualIDRegistry.getType(nextLinkDescriptor
