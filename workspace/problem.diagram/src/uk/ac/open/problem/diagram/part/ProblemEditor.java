...
public class ProblemEditor extends XtextEditor {
	...
	public void performSave(boolean overwrite, IProgressMonitor monitor) {
		...
		if (file!=null && file.exists()) { 		
			SaveAsDialog fd = new SaveAsDialog(PlatformUI.getWorkbench().getDisplay().getActiveShell());
			fd.setOriginalName(file.getName());
			fd.setTitle("Please specify the diagram name to save!");
			fd.open();
			path = fd.getResult();
			if (path != null) {
//				diagramURI = URI.createURI(filename.substring(0, filename.lastIndexOf("/")) + "/" + path.lastSegment());
