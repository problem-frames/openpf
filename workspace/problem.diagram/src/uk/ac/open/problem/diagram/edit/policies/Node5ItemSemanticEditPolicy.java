...
import uk.ac.open.problem.diagram.edit.commands.Link4CreateCommand;
import uk.ac.open.problem.diagram.edit.commands.Link4ReorientCommand;
 ...
 * @generated
 */
public class Node5ItemSemanticEditPolicy extends
		ProblemBaseItemSemanticEditPolicy {

	/**
	 * @generated
	 */
	public Node5ItemSemanticEditPolicy() {
		super(ProblemElementTypes.Node_2005);
	}
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (ProblemVisualIDRegistry.getVisualID(incomingLink) == Link4EditPart.VISUAL_ID) {
				DestroyElementRequest r = new DestroyElementRequest(
						incomingLink.getElement(), false);
				cmd.add(new DestroyElementCommand(r));
				cmd.add(new DeleteCommand(getEditingDomain(), incomingLink));
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (ProblemVisualIDRegistry.getVisualID(outgoingLink) == Link4EditPart.VISUAL_ID) {
				DestroyElementRequest r = new DestroyElementRequest(
						outgoingLink.getElement(), false);
				cmd.add(new DestroyElementCommand(r));
				cmd.add(new DeleteCommand(getEditingDomain(), outgoingLink));
	...
	protected Command getStartCreateRelationshipCommand(
		...
		if (ProblemElementTypes.Link_4004 == req.getElementType()) {
			return getGEFWrapper(new Link4CreateCommand(req, req.getSource(),
					req.getTarget()));
		}
		return null;
	}
	...
	protected Command getReorientRelationshipCommand(
		...
		case Link4EditPart.VISUAL_ID:
			return getGEFWrapper(new Link4ReorientCommand(req));
		}
