<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template name="binaryRelation">
		...
		<xsl:choose>
			...
			<xsl:when test="$predicate='alleviationa'" />
			<xsl:otherwise>
				<xsl:value-of select="$predicate"/>
				<xsl:text >(</xsl:text>
					<xsl:value-of select="translate($firstObject,$oldName,$newName)"/>
				<xsl:if test="string-length(translate($secondObject,$oldName,$newName))>0">
					<xsl:text >,</xsl:text>
	...
	<xsl:template name="contributionSwitch">
		...
		<xsl:choose>
			<xsl:when test="$rType='ALLEVIATION'">
				<xsl:text>alleviation</xsl:text>
			</xsl:when>
