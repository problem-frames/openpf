...
public class LinkCreateCommand extends EditElementCommand {
	...
	public boolean canExecute() {
		...
		if (target != null && false == target instanceof SplitCommas) {
			return false;
		}
		if (getSource() == null) {
	...
	protected CommandResult doExecuteWithResult(IProgressMonitor monitor,
		...
		Link newElement = ArgumentFactory.eINSTANCE.createLink();
		getContainer().getLinks().add(newElement);
		newElement.setAfrom(getSource());
		newElement.setToNode1(getTarget());
	 ...
	 * @generated
	 */
	protected SplitCommas getTarget() {
		return (SplitCommas) target;
	}
