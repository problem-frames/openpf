...
import argument.diagram.part.ArgumentDiagramUpdater;
import argument.diagram.part.ArgumentLinkDescriptor;
import argument.diagram.part.ArgumentNodeDescriptor;
...
public class ArgumentDiagramCanonicalEditPolicy extends CanonicalEditPolicy {
	...
	protected EStructuralFeature getFeatureToSynchronize() {
		return argument.argument.ArgumentPackage.eINSTANCE.getArgumentDiagram_Nodes();
	}
	...
	protected boolean isOrphaned(Collection<EObject> semanticChildren,
		...
		if (view.getEAnnotation("Shortcut") != null) { //$NON-NLS-1$
			return ArgumentDiagramUpdater.isShortcutOrphaned(view);
		}
	...
	private boolean isMyDiagramElement(View view) {
		return ArgumentEditPart.VISUAL_ID == ArgumentVisualIDRegistry
				.getVisualID(view);
	}
	...
	protected void refreshSemantic() {
		...
		for (View v : getViewChildren()) {
			...
			if (v.getEAnnotation("Shortcut") != null && ArgumentDiagramUpdater.isShortcutOrphaned(v)) { //$NON-NLS-1$
				orphaned.add(v);
			}
		}
		...
		for (Iterator<ArgumentNodeDescriptor> descriptorsIterator = childDescriptors
				.iterator(); descriptorsIterator.hasNext();) {
			ArgumentNodeDescriptor next = descriptorsIterator.next();
			String hint = ArgumentVisualIDRegistry.getType(next.getVisualID());
		...
		for (ArgumentNodeDescriptor next : childDescriptors) {
			String hint = ArgumentVisualIDRegistry.getType(next.getVisualID());
	...
	private Collection<IAdaptable> refreshConnections() {
		...
		Collection<ArgumentLinkDescriptor> linkDescriptors = collectAllLinks(
				getDiagram(), domain2NotationMap);
		Collection existingLinks = new LinkedList(getDiagram().getEdges());
		for (Iterator linksIterator = existingLinks.iterator(); linksIterator
				.hasNext();) {
			Edge nextDiagramLink = (Edge) linksIterator.next();
			int diagramLinkVisualID = ArgumentVisualIDRegistry
			...
			for (Iterator<ArgumentLinkDescriptor> linkDescriptorsIterator = linkDescriptors
					.iterator(); linkDescriptorsIterator.hasNext();) {
				ArgumentLinkDescriptor nextLinkDescriptor = linkDescriptorsIterator
						.next();
	 ...
	 * @generated
	 */
	private Collection<ArgumentLinkDescriptor> collectAllLinks(View view,
		...
		case Argument2EditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(ArgumentDiagramUpdater
						.getArgument_3001ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
		...
		case Argument3EditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(ArgumentDiagramUpdater
						.getArgument_3002ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
	...
	private Collection<IAdaptable> createConnections(
			Collection<ArgumentLinkDescriptor> linkDescriptors,
			Map<EObject, View> domain2NotationMap) {
		LinkedList<IAdaptable> adapters = new LinkedList<IAdaptable>();
		for (ArgumentLinkDescriptor nextLinkDescriptor : linkDescriptors) {
			...
			CreateConnectionViewRequest.ConnectionViewDescriptor descriptor = new CreateConnectionViewRequest.ConnectionViewDescriptor(
					...
					ArgumentVisualIDRegistry.getType(nextLinkDescriptor
